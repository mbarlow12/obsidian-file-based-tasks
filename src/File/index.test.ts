...
import {getFileTaskState, hashTaskCache} from "./index";
...
test('Basic representation', () => {
    const obj = getFileTaskState(cacheMetadata1, testContents1);
...
test('Representation with ids', () => {
    const obj = getFileTaskState(cacheMetadata2, testContents2);
    const expected: FileTaskCache = {
        0: {
            id: 12345,
            name: 't1',
...
test('Test Diff', async () => {
    ...
    const tCache1 = getFileTaskState(cache1, contents1);
    const tCache2 = getFileTaskState(cache2, contents2);
...
test('Test filecontents', async () => {
    ...
    const fc1 = getFileTaskState(c1.cache, c1.contents);
    const fc2 = getFileTaskState(c2.cache, c2.contents);
